...
export default function AdminDashboard() {
  ...
  const totalRevenue = enrollments
    ...
    .reduce((sum, e) => {
      const p = programs.find(
        (x) => x.id === e.programId || (x._id && x._id === e.programId)
      );
  ...
  // check admin auth (your existing logic)
  useEffect(() => {
    ...
    if (authStatus === "true" && sessionStatus === "active") {
      ...
      // If not authed, redirect to login route you use
  ...
  // Fetch enrollments & programs using your existing API routes
  const fetchData = async () => {
    setLoading(true);
    setError(null);
    try {
      // fetch all enrollments (you can change limit if you want pagination)
      const enrollRes = await fetch("/api/enrollments?limit=1000");
      if (!enrollRes.ok) {
        throw new Error(`Enrollments API error: ${enrollRes.status}`);
      }
      const enrollJson = await enrollRes.json();
      const enrollData: Enrollment[] = Array.isArray(enrollJson.data)
        ? enrollJson.data
        : enrollJson.data || [];
      ...
      if (!progRes.ok) {
        // fallback: some projects expose programs at /api/courses/programs — try that
        const altRes = await fetch("/api/courses/programs");
        if (!altRes.ok) throw new Error("Programs API error");
        const altJson = await altRes.json();
        setPrograms(altJson.data || []);
      } else {
        const progJson = await progRes.json();
        setPrograms(progJson.data || []);
      }

      // Normalize enrollments: ensure string dates
      const normalized = enrollData.map((e) => ({
        ...e,
        enrollmentDate: e.enrollmentDate
          ? new Date(e.enrollmentDate).toISOString()
  ...
  const filtered = enrollments.filter((en) => {
    ...
    const matchesSearch =
      !q ||
      en.firstName?.toLowerCase().includes(q) ||
      en.lastName?.toLowerCase().includes(q) ||
      en.email?.toLowerCase().includes(q) ||
      en.phone?.toLowerCase().includes(q) ||
      (
        programs.find((p) => p.id === en.programId || p._id === en.programId)
          ?.title || ""
      )
        .toLowerCase()
        .includes(q) ||
      en.programId?.toLowerCase().includes(q);

    const matchesStatus = statusFilter === "all" || en.status === statusFilter;
    const matchesProgram =
      programFilter === "all" || en.programId === programFilter;
  ...
  const exportToCSV = () => {
    ...
    const csvData = filtered.map((e) => {
      const programTitle =
        programs.find((p) => p.id === e.programId || p._id === e.programId)
          ?.title ||
        e.programId ||
        "";
      return [
        e.enrollmentId || e._id || "",
        ...
        e.status,
        e.paymentStatus,
        e.enrollmentDate ? new Date(e.enrollmentDate).toLocaleString() : "",
        e.city || "",
        e.state || "",
        e.education || "",
        e.experience || "",
        (e.motivation || "").replace(/\n/g, " "),
      ];
    });
      ...
      <AnimatedElement animation="fade-up">
        <div className="mb-8">
          ...
          <div className="space-y-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold gradient-text-primary">
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {filtered.map((en, i) => {
                  const programTitle =
                    programs.find(
                      (p) => p.id === en.programId || p._id === en.programId
                    )?.title ||
                    en.programId.replace?.("-", " ") ||
                    en.programId;

                  return (
                    <AnimatedElement
                      key={en._id ?? en.enrollmentId ?? i}
                      animation="scale-in"
                      delay={0.06 * i}
                    >
                        ...
                        <CardContent className="p-6">
                          <div className="flex items-start justify-between">
                            <div className="space-y-3 flex-1">
                              ...
                              {en.motivation && (
                                <div className="text-sm bg-gray-50 p-3 rounded-lg">
                                  ...
                                  <p className="mt-1">{en.motivation}</p>
                                </div>
                              )}
                            </div>
                            ...
                            <div className="text-right text-sm text-muted-foreground ml-6">
                              ...
                              {en.education && (
                                <p className="capitalize">🎓 {en.education}</p>
                              )}
                              {en.experience && (
                                <p>💼 {en.experience} experience</p>
                              )}
                              {en.whatsapp && <p>📱 WhatsApp: {en.whatsapp}</p>}
                              {en.linkedin && (
                                <p>
                                  💼{" "}
                                  <a
                      ...
                      <CardContent className="space-y-4">
                        <div className="flex justify-between items-center">
                          ...
                          <Badge variant="secondary">
                            {programEnrollments.length} enrolled
                          </Badge>
                        </div>
                        <div className="text-sm text-muted-foreground">
                  ...
                  <div className="space-y-3">
                    {["pending", "confirmed", "completed", "cancelled"].map(
                      (status) => {
                        ...
                        const percentage =
                          enrollments.length > 0
                            ? ((count / enrollments.length) * 100).toFixed(1)
                            : "0";
                        return (
                          <div
                            key={status}
